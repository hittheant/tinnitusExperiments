...
# Create an outlet to broadcast the markers
...
expName = 'AuditoryANT'  # from the Builder filename that created this script
...
def setupDevices(expInfo, thisExp, win):
    ...
    # create speaker 'orienting_cue'
    deviceManager.addDevice(
        deviceName='orienting_cue',
        deviceClass='psychopy.hardware.speaker.SpeakerDevice',
        index=-1
    )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    target = sound.Sound(
        ...
        stereo=True, 
        hamming=True, 
        speaker='target',    name='target'
    )
    target.setVolume(1.0)
    key_resp = keyboard.Keyboard(deviceName='key_resp')
    orienting_cue = sound.Sound(
        'A', 
        secs=0.1, 
        stereo=True, 
        hamming=True, 
        speaker='orienting_cue',    name='orienting_cue'
    )
    orienting_cue.setVolume(1.0)
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[fixation_cross, target, key_resp, orienting_cue],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        target.setSound(target_file, secs=1.7, hamming=True)
        ...
        while continueRoutine:
            ...
            if fixation_cross.status == STARTED:
                ...
                if tThisFlipGlobal > fixation_cross.tStartRefresh + 3.2-frameTolerance:
            ...
            if target.status == NOT_STARTED and tThisFlip >= iti+0.1+0.4-frameTolerance:
                # keep track of start time/frame for later
            ...
            if target.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > target.tStartRefresh + 1.7-frameTolerance or target.isFinished:
                    # keep track of stop time/frame for later
            ...
            if key_resp.status == NOT_STARTED and tThisFlip >= iti-frameTolerance:
                # keep track of start time/frame for later
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                theseKeys = key_resp.getKeys(keyList=['up','down'], ignoreKeys=["escape"], waitRelease=False)
            ...
            # *orienting_cue* updates
            
            # if orienting_cue is starting this frame...
            if orienting_cue.status == NOT_STARTED and tThisFlip >= iti-frameTolerance:
                # keep track of start time/frame for later
                orienting_cue.frameNStart = frameN  # exact frame index
                orienting_cue.tStart = t  # local t and not account for scr refresh
                orienting_cue.tStartRefresh = tThisFlipGlobal  # on global time
                # add timestamp to datafile
            ...
            if orienting_cue.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > orienting_cue.tStartRefresh + 0.1-frameTolerance or orienting_cue.isFinished:
                    # keep track of stop time/frame for later
                    orienting_cue.tStop = t  # not accounting for scr refresh
                    orienting_cue.tStopRefresh = tThisFlipGlobal  # on global time
                    orienting_cue.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
            ...
            if thisExp.status == PAUSED:
                pauseExperiment(
                    ...
                    win=win, 
                    timers=[routineTimer], 
                    playbackComponents=[target, orienting_cue]
                )
        ...
        orienting_cue.pause()  # ensure sound has stopped at end of Routine
        # the Routine "trial" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed 4.0 repeats of 'trials'
    outlet.push_sample(['blockEnd', str(0.0)])
    ...
    outlet.push_sample(['experimentEnd', str(0.0)])
    endExperiment(thisExp, win=win)
